...
export default function Impossible(): JSX.Element {
    const [num, set_num] = useState<number>(random_num);
    const [counter, set_counter] = useState<number>(4);
    ...
    function handle_click(): void {
        if(inp.current){
            ...
            if(val == num){
                set_wins(wins + 1);
                localStorage.setItem("imp_wins", (wins + 1).toString());
            ...
            if(counter == 1 && val != num){
                set_loses(loses + 1);
                localStorage.setItem("imp_loses", (loses + 1).toString());
    ...
    useEffect(function (): void {
        const saved_wins: string = localStorage.getItem("imp_wins") || "0";
        const saved_loses: string = localStorage.getItem("imp_loses") || "0";
    ...
    return (
        <div className={styles.page}>
            ...
            <Link href={"../extreme"} className={styles.back_btn}> Back </Link>
            <Link href={"../one-time"} className={styles.next_btn}> Next </Link>
            ...
            <main className={styles.main}>
                <h1 className={styles.main_title}>Impossible Mode</h1>
                <div className={styles.game_box}>
                    <div className={player_status != "You Win." && player_status != "You Lose." ? styles.info_box : styles.none}>
                        ...
                        <p className={styles.rules_p}>
                            ...
                            <br />
                            You Have <span className={`${styles.bold} ${styles.italic}`}> 4 Times Only.</span>
                        </p>
