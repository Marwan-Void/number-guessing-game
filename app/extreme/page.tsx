...
export default function Extreme(): JSX.Element {
    ...
    const [counter, set_counter] = useState<number>(Attempts.Extreme);
    ...
    function handle_click(): void {
        if(inp.current){
            ...
            if(val == num){
                ...
                set_player_status(`Between 0 and ${Range.Hard_Extreme - 1} ( also can be ${Range.Hard_Extreme - 1} or 0 )`);
            }
            else if(val > num){
            ...
            if(counter == 1 && val != num){
                set_loses(loses + 1);
                localStorage.setItem("ext_loses", (loses + 1).toString());
                set_player_status("Lose.");
            }
        }
    }
    function handle_again(): void {
        set_counter(Attempts.Extreme);
        set_player_status("");
    ...
    useEffect(function (): void {
        const saved_wins: string = localStorage.getItem("ext_wins") || "0";
        const saved_loses: string = localStorage.getItem("ext_loses") || "0";
        setTimeout(function (){
    ...
    return (
        <div className={styles.page}>
            ...
            <Link href={"../hard"} className={styles.back_btn}> Back </Link>
            <Link href={"../impossible"} className={styles.next_btn}> Next </Link>
            ...
            <main className={styles.main}>
                <h1 className={player_status != "Win." && player_status != "Lose." ? styles.main_title : styles.none}>Extreme Mode</h1>
                <div className={styles.game_box}>
                    <div className={player_status != "Win." && player_status != "Lose." ? styles.info_box : styles.none}>
                        ...
                        <p className={styles.rules_p}>
                            ...
                            <br />
                            You Have <span className={`${styles.bold} ${styles.italic}`}> {Attempts.Extreme} Attempts Only.</span>
                        </p>
